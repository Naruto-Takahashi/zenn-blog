...
2. コミットメッセージの自動生成
ステージングされた状態で`CTRL + G`を押すとコミットメッセージの自動生成が開始します．
 ![](/images/lazygit-gemini/usage-02.png)
3. メッセージの選択
英語と日本語それぞれで，簡易版と詳細版の2つずつのコミットメッセージが生成されます．`j`，`k`や矢印キーで選択し，`ENTER`で決定します．
![](/images/lazygit-gemini/usage-03.png)
4. コミット
下のようなメッセージの編集画面に移るので，(必要に応じて) 選択したメッセージを編集します．(中止したい場合は，`ESC`や`CTRL + C`を押します) ．`CTRL + D`で決定され，そのままコミットされます．
